      ...
      <div className="dept-doctor-card">
            ...
            <div className="others_info_doc">
              ...
              {doctor?.qualification?.length > 0 && (
                <div className="doctor-degree">
                  <p>
                    {doctor?.qualification[0]?.degree}{" "}
                    {doctor?.qualification[0]?.year}
                  </p>
                </div>
              )}
        ...
        <div className="doctor-details">
          <div className="doctor-rating">
            <p>
              <HiStar fontSize={15} color="#ffd700" />{" "}
              {doctor?.rating || "Rating"}
            </p>
          </div>
          <div className="doctor-experience">
            <p>
              <FaUserMd fontSize={15} /> {doctor?.experience || ""}+yrs
            </p>
          </div>
        </div>
        ...
        <div className="doctor_speciality mb-2">
          <div className=" d_spl">
            {doctor?.speciality
              ?.slice(0, showAllSpecialities ? doctor.speciality.length : 1)
            ...
            {doctor?.speciality?.length > 1 && (
              <div className="show-more-btn-container ">
                ...
                >
                  {showAllSpecialities
                    ? "Show Less"
                    : `+${doctor.speciality.length - 1} More`}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="doctor-location">
          <p>
            <IoLocationSharp fontSize={15} color="red" /> {doctor?.region || ""}
          </p>
        </div>

        <div className="call-to-action">
          <button onClick={handleCall} className="btn-border-to-bg">
            Contact
          </button>
          <button
            onClick={() => navigate(`/docprofile/${doctor?.slug}`)}
            className="btn-bg-change"
          >
            Book Appointment
          </button>
        </div>
